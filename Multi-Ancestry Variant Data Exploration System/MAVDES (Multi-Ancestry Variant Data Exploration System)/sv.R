...
option_list <- list(
  ...
  make_option(c("-o","--output_dir"), type="character", default="sv_results",
...
generate_sv_summary_plots <- function(
  ...
  message("Preparing SV summary plots")
...
if (2 %in% steps || 3 %in% steps || 4 %in% steps || 5 %in% steps) {
  ...
  df <- analyze_vep_sv_variants(
    file.path(input_dir, "sv", paste0(gene_name, "_sv_vep_output.vcf"))
  )
...
if (1 %in% steps) {
  ...
  sv_data <- fread(file.path(input_dir, "sv", paste0(gene_name,"_sv_raw.vcf")), data.table = FALSE)
