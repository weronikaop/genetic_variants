...
Examples:
  extract.sh -r chr14:105586437-106879843 -i input.vcf.gz -o chr14_snvs.vcf.gz
...
gff_file <- "part2/chm13v2.0_RefSeq_Liftoff_v5.2.gff3.gz"

#txdb <- makeTxDbFromGFF(gff_file, format="gff3", organism="Homo sapiens")

txdb_file <- "C:/Users/weronikaoprzedek/Desktop/projekt/part2/chm13v2.0_RefSeq_Liftoff_v5.2.sqlite"
...
#finds gene group regions for extract.sh
#save gene locations to xlsx

TRB_genes <- genes[grepl("^TRBJ|^TRBV|^TRBC|^TRBD", mcols(genes)$gene_id) & !grepl("^IGHMBP", mcols(genes)$gene_id)]

#clean from ORs
TRB_genes <- TRB_genes[!grepl("OR", mcols(TRB_genes)$gene_id)]

#part of probably pseudogenes are on different chr, no need for them
#TRB_genes <- TRB_genes[seqnames(TRB_genes) == "chr14"]

#table of genes
TRB_table <- data.frame(
  Type = "TRB",
  Chr = as.character(seqnames(TRB_genes)),
  Start = start(TRB_genes),
  End = end(TRB_genes),
  gene_name = mcols(TRB_genes)$gene_id
)


write.xlsx(TRB_table, "part2/location/TRB.xlsx", overwrite = TRUE)

#print region location
chr <- unique(TRB_table$Chr)
start_pos <- min(TRB_table$Start)
end_pos <- max(end(TRB_genes))
print(paste(chr, start_pos, end_pos, sep=":"))
```

```{bash}
#| eval: false
#| include: true
#Extract IgH-related SNV with Bash_script
bash extract.sh -c chr14 -s 99839969 -e 101155136 -i variants_T2T-CHM13_snv_snv_alt_HGSVC2024v1.0.vcf.gz -o T2T_snv_IGH.vcf 1> IGH_log.txt 2>&1 &

#Extract IGK-related SNV with Bash_script
bash extract.sh -c chr2 -s 88866370 -e 90790947 -i variants_T2T-CHM13_snv_snv_alt_HGSVC2024v1.0.vcf.gz -o T2T_snv_IGK.vcf 1> IGK_log.txt 2>&1 &

#Extract IgL-related SNV with Bash_script
bash extract.sh -c chr22 -s 22439629 -e 23345823 -i variants_T2T-CHM13_snv_snv_alt_HGSVC2024v1.0.vcf.gz -o T2T_snv_IGL.vcf 1> IGL_log.txt 2>&1 &

#Extract TRA-related SNV with Bash_script
bash extract.sh -c chr14 -s 15819498 -e 16749884 -i variants_T2T-CHM13_snv_snv_alt_HGSVC2024v1.0.vcf.gz -o T2T_snv_TRA.vcf 1> TRA_log.txt 2>&1 &

#Extract TRB-related SNV with Bash_script
bash extract.sh -c chr7 -s 143614080 -e 144169005 -i variants_T2T-CHM13_snv_snv_alt_HGSVC2024v1.0.vcf.gz -o T2T_snv_TRB.vcf 1> TRB_log.txt 2>&1 &
```
...
analyze_vep_snv_variants <- function(vcf_file) {
  ...
  if (length(vcf_snv) == 0) stop("No SNV variants found")
...
generate_snv_summary_plots <- function(
  ...
  plot_data$position_label <- paste0(
    plot_data$chromosome, ":", format(plot_data$position, big.mark = ",")
  )
  ...
  # 2 SNV track plot (per-sample heatmap)
  ...
  # Dashed separators between regions
  ...
  # 3 Sample summary (variant count per sample)
  ...
  p_samples <- ggplot(sample_summary_data,
    ...
    geom_col() +
    scale_fill_manual(values = type_colors, name = "Variant Type") +
    labs(title = paste(gene_name, "— Variant Count by Sample"),
         x = "Sample", y = "Count") +
    theme_minimal() +
    theme(axis.text.x = element_markdown(angle = 45, hjust = 1,
                                        color = region_colors[sample_summary_data$Region]))
  
  # 4 Variant-type distribution (facet-able)
  type_plot_data <- plot_data %>%
    count(Variant_Type) %>%
    mutate(Gene = gene_name) # Add gene column for later faceting
  ...
  p_types <- ggplot(type_plot_data, aes(x = reorder(Variant_Type, n), y = n, fill = Variant_Type)) +
    ...
    labs(title = paste(gene_name, "— Variant Type Distribution"),
         x = "Variant Type", y = "Count") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          legend.position = "none")
  
  # 5 Average variants per region
  ...
  p_regions <- ggplot(region_plot_data,
                      aes(x = Region, y = mean_mut, fill = Variant_Type)) +
    geom_col() +
    scale_fill_manual(values = type_colors) +
    labs(title = paste(gene_name, "— Average Variants per Sample by Region"),
         x = "Region", y = "Mean mutations per sample") +
    theme_minimal()
  ...
  if (!is.null(output_prefix)) {
    ...
    ggsave(paste0(output_prefix, "_regions.png"), p_regions, width = 8, height = 6, dpi = 300)
  }
  
  # 7 Save processed data for later faceted plots
...
bcftools annotate   --rename-chrs <(echo -e "chr1\t1\nchr2\t2\nchrX\tX\nchrY\tY")   GENE_snv_raw.vcf   -Oz -o GENE_snv_raw_ncbi.vcf.gz

bcftools annotate   --rename-chrs chr_map.txt   -o GENE_snv_raw_ncbi_renamed.vcf.gz   -O z GENE_snv_raw_ncbi.vcf.gz

perl ensembl-vep-release-115.1/vep \
  --input_file snv/GENE_snv_raw_ncbi_renamed.vcf.gz \
  --output_file snv/vep_output/GENE_snv_vep_output.vcf \
  --format vcf \
...
option_list <- list(
  ...
  make_option(c("-i","--input_dir"), type="character", default="part2",
              help=paste(
                ...
                "\n  snv/T2T_snv_<GENE>.vcf",
                sep="\n")),
  make_option(c("-o","--output_dir"), type="character", default="part2/graphs/igl_snv",#---
              help="Output folder path"),
  make_option(c("-s","--step"), type="character", default="1,2,3,4,5",
              help="Step(s) to run: 1 = Variant burden heatmap, 2 = Bubble map plots, 3 = Bubble: synonymous vs nonsynonymous, 4 = Summary plots, 5 = Summary plots without modifier consequences, or combination like 1,2,3 (default: %default)"),
...
locus <- load_gene_information(
  file.path(input_dir, "location", paste0(gene_name, ".xlsx"))
)
...
if (any(c(2, 3, 4, 5) %in% steps)) {
  ...
  df <- analyze_vep_snv_variants(
    file.path(input_dir, "snv", "vep_output", paste0(gene_name, "_snv_vep_output.vcf"))
  )
...
if (1 %in% steps) {
  ...
  snv_data <- fread(file.path(input_dir, "snv", paste0("T2T_snv_",gene_name,".vcf")), data.table = FALSE)
...
## 3. Bubble: synonymous vs nonsynonymous
...
if (4 %in% steps) {
  ...
  summary_plots <- generate_snv_summary_plots(
    ...
    gene_name      = gene_name,
    output_prefix  = file.path(output_dir, paste0(gene_name, "_snv_summary"))
  )
...
if (5 %in% steps) {
  ...
  summary_plots_focus <- generate_snv_summary_plots(
    ...
    output_prefix  = file.path(output_dir, paste0(gene_name, "_snv_summary_focus"))
  )
