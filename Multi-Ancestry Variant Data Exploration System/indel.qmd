...
#padding different than in sv (2000, delete it later)
#Extract IgH-related indel with Bash_script
bash extract.sh -c chr14 -s $((99839969 - PADDING)) -e 101155136 -i variants_T2T-CHM13_indel_insdel_alt_HGSVC2024v1.0.vcf.gz -o T2T_indel_IGH.vcf 1> IGH_log.txt 2>&1 &

#Extract IGK-related indel with Bash_script
bash extract.sh -c chr2 -s $((88866370 - PADDING)) -e 90790947 -i variants_T2T-CHM13_indel_insdel_alt_HGSVC2024v1.0.vcf.gz -o T2T_indel_IGK.vcf 1> IGK_log.txt 2>&1 &

#Extract IGL-related indel with Bash_script
bash extract.sh -c chr22 -s $((22439629 - PADDING)) -e 23345823 -i variants_T2T-CHM13_indel_insdel_alt_HGSVC2024v1.0.vcf.gz -o T2T_indel_IGL.vcf 1> IGL_log.txt 2>&1 &

#Extract TRA-related indel with Bash_script
bash extract.sh -c chr14 -s $((15819498 - PADDING)) -e 16749884 -i variants_T2T-CHM13_indel_insdel_alt_HGSVC2024v1.0.vcf.gz -o T2T_indel_TRA.vcf 1> TRA_log.txt 2>&1 &

#Extract TRB-related indel with Bash_script
bash extract.sh -c chr7 -s $((143614080 - PADDING)) -e 144169005 -i variants_T2T-CHM13_indel_insdel_alt_HGSVC2024v1.0.vcf.gz -o T2T_indel_TRB.vcf 1> TRB_log.txt 2>&1 &
```
...
analyze_vep_indel_variants <- function(vcf_file) {
  ...
  if ("SVTYPE" %in% colnames(info_df)) {
    ...
    svtype_info <- rep("UNK", length(vcf_indel))
  }
  ...
  # Ensure all haplotypes exist as columns
  ...
  # Variant-level overall category vector
  ...
  cat("Haplotype-based INDEL analysis completed.\n")
...
plot_indel_bubble <- function(
  ...
  indel_long <- indel_result %>%
    ...
    mutate(
      ...
      SVLEN = abs(SVLEN),               #<- IMPORTANT: abs for length
      chromosome = as.character(chromosome),
...
generate_indel_summary_plots <- function(
  ...
  message("Generating INDEL summary plots for gene: ", gene_name)
  ...
  # Pretty position labels
  plot_data$position_label <- paste0(plot_data$chromosome, ":", format(plot_data$position, big.mark = ","))
  ...
  y_label_colors <- region_colors[
    as.character(sample_regions$Region[match(levels(plot_data$Sample), sample_regions$Sample)])
  ]
  
  # 2 Main INDEL variant track
  ...
  # Invisible points to create Region legend
  p_main <- p_main +
    geom_point(
      data = sample_regions,
  ...
  if (n_pos > 50) {
    ...
    p_main <- p_main +
      scale_x_discrete(breaks = all_labels$Position[idx], labels = all_labels$position_label[idx])
  } else {
    p_main <- p_main +
      scale_x_discrete(labels = setNames(all_labels$position_label, all_labels$Position))
  }
  ...
  p_samples <- ggplot(sample_summary_data,
    ...
    geom_col() +
    scale_fill_manual(values = type_colors, name = "Variant Type") +
    labs(title = paste(gene_name, "— INDEL Count by Sample"),
         x = "Sample", y = "Count") +
    theme_minimal() +
    theme(axis.text.x = element_markdown(angle = 45, hjust = 1, color = x_label_colors)) +
    geom_point(
  ...
  # 4 Variant type distribution (for faceting later)
  type_plot_data <- plot_data %>%
    count(Variant_Type) %>%
    mutate(Gene = gene_name)
  
  p_types <- ggplot(type_plot_data, aes(x = reorder(Variant_Type, n), y = n, fill = Variant_Type)) +
    geom_col() +
    geom_text(aes(label = n), vjust = -0.4, size = 3) +
    scale_fill_manual(values = type_colors) +
    labs(title = paste(gene_name, "— INDEL Type Distribution"),
         x = "Variant Type", y = "Count") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          legend.position = "none")
  
  # 5 Average INDELs per region (for faceting)
  ...
  p_region <- ggplot(region_plot_data,
    ...
    geom_col() +
    scale_fill_manual(values = type_colors) +
    labs(title = paste(gene_name, "— Average INDELs per Sample by Region"),
         x = "Region", y = "Mean INDELs per sample") +
    theme_minimal()
  ...
  # 7 Save processed data for later faceting
  if (save_data && !is.null(output_prefix)) {
    saveRDS(plot_data, file = paste0(output_prefix, "_plot_data.rds"))
    saveRDS(type_plot_data, file = paste0(output_prefix, "_type_data.rds"))
    saveRDS(region_plot_data, file = paste0(output_prefix, "_region_data.rds"))
  }
  ...
  return(list(
    ...
    plot_data   = plot_data,
    type_data   = type_plot_data,
    region_data = region_plot_data
  ))
}
```
...
bcftools annotate   --rename-chrs <(echo -e "chr1\t1\nchr2\t2\nchrX\tX\nchrY\tY")   T2T_indel_TRB.vcf   -Oz -o T2T_indel_TRB_ncbi.vcf.gz

bcftools annotate   --rename-chrs chr_map.txt   -o T2T_indel_TRB_ncbi_renamed.vcf.gz   -O z T2T_indel_TRB_ncbi.vcf.gz

cd ..

perl ensembl-vep-release-115.1/vep \
  --input_file indel/T2T_indel_TRB_ncbi_renamed.vcf.gz \
  --output_file indel/vep_output/TRB_indel_vep_filtered.vcf \
  --format vcf \
...
vcf_data <- read_tsv("part2/indel/vep_output/IGH_indel_vep_filtered.vcf",
...
vcf_header <- readLines("part2/indel/T2T_indel_IGH.vcf")[grepl("^#", readLines("part2/indel/T2T_indel_IGH.vcf"))]
write.table(vcf_filter,
            "part2/indel/vep_output/IGH_indel_vep_filtered.vcf",
...
option_list <- list(
  ...
  make_option(c("-i","--input_dir"), type="character", default="part2",
              help=paste(
                ...
                "\n  indel/T2T_indel_<GENE>.vcf",
                sep="\n")),
  make_option(c("-o","--output_dir"), type="character", default="part2/graphs/igl_indel",
              help="Output folder path"),
  make_option(c("-s","--step"), type="character", default="1,2,3,4,5,6",
              help="Step(s) to run: 1 = Variant burden heatmap, 2 = Bubble map plots, 3 = Bubble: PTV vs non-PTV, 4 = Summary plots, 5 = Summary plots without modifier consequences, 6 = Track map or combination like 1,2,3 (default: %default)"),
...
if (2 %in% steps || 3 %in% steps || 4 %in% steps || 5 %in% steps) {
  ...
  df <- analyze_vep_indel_variants(file.path(input_dir,"indel","vep_output",paste0(gene_name,"_indel_vep_filtered.vcf")))
...
if (1 %in% steps) {
  ...
  indel_data <- fread(file.path(input_dir, "indel", paste0("T2T_indel_",gene_name,".vcf")), data.table = FALSE)
